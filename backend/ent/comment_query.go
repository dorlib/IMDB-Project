...
func (cq *CommentQuery) QueryReview() *ReviewQuery {
	query := &ReviewQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithReview(opts ...func(*ReviewQuery)) *CommentQuery {
	query := &ReviewQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if cq.withUser != nil || cq.withReview != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withReview; query != nil {
		if err := cq.loadReview(ctx, query, nodes, nil,
			func(n *Comment, e *Review) { n.Edges.Review = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadReview(ctx context.Context, query *ReviewQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Review)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].comment_review == nil {
			continue
		}
		fk := *nodes[i].comment_review
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "comment_review" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
