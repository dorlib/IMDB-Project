...
func (uq *UserQuery) QueryMovies() *MovieQuery {
	query := &MovieQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithMovies(opts ...func(*MovieQuery)) *UserQuery {
	query := &MovieQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withMovies; query != nil {
		if err := uq.loadMovies(ctx, query, nodes,
			func(n *User) { n.Edges.Movies = []*Movie{} },
			func(n *User, e *Movie) { n.Edges.Movies = append(n.Edges.Movies, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withDirectors; query != nil {
		if err := uq.loadDirectors(ctx, query, nodes,
			func(n *User) { n.Edges.Directors = []*Director{} },
			func(n *User, e *Director) { n.Edges.Directors = append(n.Edges.Directors, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadReviews(ctx context.Context, query *ReviewQuery, nodes []*User, init func(*User), assign func(*User, *Review)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_reviews" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadMovies(ctx context.Context, query *MovieQuery, nodes []*User, init func(*User), assign func(*User, *Movie)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Movie(func(s *sql.Selector) {
		s.Where(sql.InValues(user.MoviesColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (uq *UserQuery) loadDirectors(ctx context.Context, query *DirectorQuery, nodes []*User, init func(*User), assign func(*User, *Director)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
