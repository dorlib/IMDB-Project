...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withComments; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_comments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_comments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
	...
	if query := uq.withLikes; query != nil {
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.LikesPrimaryKey[0], fks...))
			},
	...
	if query := uq.withAchievements; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Achievements = []*Achievement{}
		}
		query.withFKs = true
		query.Where(predicate.Achievement(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AchievementsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_achievements" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_achievements" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Achievements = append(node.Edges.Achievements, n)
		}
	}
