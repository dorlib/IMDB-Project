...
func (*Movie) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case movie.FieldTitle, movie.FieldDescription, movie.FieldGenre, movie.FieldImage, movie.FieldCreatedAt:
...
// QueryActor queries the "actor" edge of the Movie entity.
func (m *Movie) QueryActor() *ActorQuery {
	return (&MovieClient{config: m.config}).QueryActor(m)
}
